...
- **Modular Addons**: Optional complex features like Personal Context and the Meeting Assistant are built in an isolated `modules/` folder with minimal integration hooks in `main.py` and `app.py`. They bring their own storage and UI logic while reusing core providers.
...
- `modules/`: Fully isolated addons (Personal Context, Meeting Assistant) that are injected into the core UI.
...
- Meeting Assistant WASAPI loopback capture is highly Windows-specific and requires appropriate drivers.
...
- **[2026-06-04]**: Implemented Personal Context System and Meeting Assistant modules as isolated addons. Added DOCX/PDF parsing and WASAPI desktop audio capture.
